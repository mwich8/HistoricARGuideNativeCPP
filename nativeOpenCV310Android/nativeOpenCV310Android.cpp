...
extern "C" {
	...
	JNIEXPORT void JNICALL Java_com_tum_historicarguide_MainActivity_nativeOpticalDetectionDebug(JNIEnv *env, jobject instance, jlong addrRgba, jfloat fps) {
		...
		opticalDetectionDebug(mRgb, equalizedMat);
...
Rect createRect(Point p1, Point p2, int expandVal) {
	Rect tempRect = Rect(p1, p2);
	if (expandVal == 0) {
		int pointDistance = tempRect.br().y - tempRect.tl().y;
		Point topLeftPoint = Point(max(tempRect.tl().x - (pointDistance / 2), 0), max(tempRect.tl().y - (pointDistance / 2), 0));
		Point bottomRightPoint = Point(min(tempRect.br().x + (pointDistance / 2), frameWidth - 1), min(tempRect.br().y + (pointDistance / 2), frameHeight - 1));
		Rect searchRect = Rect(topLeftPoint, bottomRightPoint);
		return searchRect;
	}
	else {
		Point topLeftPoint = Point(max(tempRect.tl().x - expandVal, 0), max(tempRect.tl().y - expandVal, 0));
		Point bottomRightPoint = Point(min(tempRect.br().x + expandVal, frameWidth - 1), min(tempRect.br().y + expandVal, frameHeight - 1));
		Rect searchRect = Rect(topLeftPoint, bottomRightPoint);
		return searchRect;
	}
}
...
void mapARImageOnMiddleArch(Mat mRgb, vector<vector<Point>> contours, float middleArchDiameter) {
	...
	if (!middleArch.empty()) {
		mapARImageOnZenithOfMiddleArch(mRgb, middleArch, middleArchDiameter);
	} 
...
void mapARImageOnZenithOfMiddleArch(Mat mRgb, vector<Point> middleArch, float middleArchDiameter) {
	...
	float longestSide = getLongestContourSide(middleArch);
	// Only execute if longestSide is max ca. 1/3 of frameWidth
	float maxWidthPerc = 0.33f;
	if (longestSide < frameWidth * maxWidthPerc) {
		// Scale ARImage
		int rectWidth = (int)(longestSide * ((float)arImage.cols / middleArchDiameter));
		...
		// Define the percentual arch pos in the historic image
		...
		Point bottomRight = Point(min(zenithOfArch.x + (int)(rectWidth * (1.0f - archZenithPercPosX)), frameWidth), (min(zenithOfArch.y + (int)(rectHeight * (1.0f- archZenithPercPosY)), frameHeight)));
...
Point getZenithOfContour(vector<Point> contour) {
	...
	Point highestPoint = Point(0, frameHeight);
	for (Point p : contour) {
...
vector<Point> getAtPointedArch(Mat mRgb, vector<vector<Point>> contours) {
	...
	if (touchedPoint.x != 0 || touchedPoint.y != 0) {
		...
		for (vector<Point> contour : contours) {
			...
			Point bottomRight = Point(min(boundingRec.br().x + 5, frameWidth), min(boundingRec.br().y + 75, frameHeight));
	...
	int widthSum = 0;
	// Get biggest contour
	for (vector<Point> contourPointedAt : contoursPointedAt) {
		Rect boundingRec = boundingRect(contourPointedAt);
		int currentWidth = boundingRec.width;
		widthSum += currentWidth;
		if (currentWidth > maxWidth && contourIsArch(contourPointedAt)) {
			if (boundingRec.width < frameWidth * 0.33) {
				if (boundingRec.width * 0.3 < boundingRec.height) {
					biggestContour = contourPointedAt;
	...
	if (!biggestContour.empty()) {
		...
		for (vector<Point> contourPointedAt : contoursPointedAt) {
			Rect boundingRec = boundingRect(contourPointedAt);
			if (boundingRec.width > minWidth) {
				potentialContours.push_back(contourPointedAt);
				potentialContoursWidthSum += boundingRec.width;
			}
		}
		if (potentialContours.size() > 0) {
			...
			last_arch_widths[last_pushed_index] = boundingRect(biggestContour).width;
		}
		
		last_pushed_index = (last_pushed_index + 1) % number_of_arch_widths;
	}
	else {
		last_arch_widths[(last_pushed_index - 1) % number_of_arch_widths] = 0;
	}
	...
	if (numberOfWidths > 2) {
		...
		if (archHasBackground) {
			...
			Point highestZenith = Point(frameWidth, frameHeight);
		...
		if (!biggestContour.empty()) {
			...
			if (avWidth * 0.8 < biggestContWidth && avWidth * 1.2 > biggestContWidth) {
...
bool pointsAtSamePos(Point p1, Point p2, float maxDeviation) {
	int maxDistance = (min(frameHeight, frameWidth)) * maxDeviation;
